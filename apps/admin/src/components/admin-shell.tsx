...
/** The pages the header links to. Adding one here puts it in every layout. */
  ...
  return (
    <header className="flex shrink-0 items-center gap-4 border-b border-rule bg-surface px-4 py-2">
      ...
      <nav className="flex items-center gap-3">
        {NAV.map((item) => (
          <Link key={item.href} href={item.href} className="text-ink-soft underline-offset-2 hover:text-accent hover:underline">
            {item.label}
          </Link>
        ))}
      </nav>
...
function Forbidden({ access }: { access: Access }) {
  return (
...
/**
 * The admin gate and the chrome around every protected page. Signed-out visitors
 * go to `/connexion`; signed-in non-admins get told why they see nothing. Writes
 * are protected by RLS regardless of what this renders.
 *
 * `fill` is for the graph editor, which owns the whole viewport; the ordinary
 * pages scroll.
 */
export async function AdminShell({ children, fill = false }: { children: ReactNode; fill?: boolean }) {
  ...
  if (outcome.render === 'FORBIDDEN') return <Forbidden access={access} />;

  const email = access.state === 'ADMIN' ? access.email : null;

  return (
    <div className={fill ? 'flex h-screen min-h-0 flex-col' : 'flex min-h-screen flex-col'}>
      <AdminHeader email={email} />
      <div className={fill ? 'min-h-0 flex-1' : 'flex-1'}>{children}</div>
    </div>
  );
}
