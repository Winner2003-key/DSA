...
| 4 | S6 | `S6-rooms.md`: rooms on two devices, lobby, QR / share, presence, Tireur-ready across devices, realtime, stale rooms, rematch | `apps/mobile/`, SQL 0008 / `05_rooms.sql`, regenerates `00` | ✅ done — lead review pending |
| 5 | S7b | `S7b-voice-in-app.md`: speak to play (Expo Go and web), hold-to-talk, calibration screen, Voix mode, fallbacks | `apps/mobile/`, small `packages/voice` additions | after S9 (both own `apps/mobile`) |
...
| any | S5b | `S5b-admin-homonyms.md`: the Homonymes page (descriptions for people sharing a name, with a Tireur card preview) | `apps/admin/` | ✅ done — lead review pending |
| 5 | S9 | `S9-timed-games.md`: **optional** timer (checkbox, 40 s thinking + 120 s game, admin Réglages), Tireur can change the name max 2×, the book's path shown when the name wasn't found | `supabase/` 0009 / `06_timer.sql`, `apps/mobile/`, `apps/admin/` Réglages, additive core | ▶ ready: after S6 and S5b are committed; not at the same time as S7b |
